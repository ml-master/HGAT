...
parser.add_argument('--node', action='store_false', default=False,
                    help='Use node-level attention or not. ')
parser.add_argument('--type', action='store_false', default=False,
...
def evaluate(preds_list, y_list):
    ...
    if multi_label:
        ...
        for key,value in classes_dict.items():
            onehot_class[tuple(value)]=key
        y_true = []
        y_pred = []
        for onehot1,onehot2 in zip(y_list,preds):
            y_true.append(onehot_class.get(tuple(onehot1)))
            y_pred.append(onehot_class.get(tuple(onehot2)))
        # 计算类别为'fake'的准确率
        precision_fake = metrics.precision_score(y_true, y_pred, pos_label='fake')
        # 计算类别为'legitimate'的准确率
        precision_legitimate = metrics.precision_score(y_true, y_pred, pos_label='real')

        return ER, F1, precision_fake, precision_legitimate

LOSS = margin_loss if 'multi' in dataset else nll_loss

class MyDataset(Dataset):
    def __init__(self, input_adj_test, input_features_test, idx_out_test, idx_test):
        super().__init__()

        self.num_sample =input_adj_test.shape[0]

        self.input_adj_test = input_adj_test
        self.input_features_test = input_features_test
        self.idx_out_test = idx_out_test
        self.idx_test = idx_test

    def __getitem(self, idx):
        return self._adj_test[idx], self.input_features_test[idx],self.idx_out_test[idx], self.idx_test[idx]
...
def train(epoch,
    ...
    acc_train, f1_train, acc_fake, acc_legitimate = evaluate(O, L)
    loss_train.backward()
    optimizer.step()
    ...
    if 'multi' in dataset:
        acc_val, res_line,  acc_fake, acc_legitimate  = results
        return float(acc_val.item()), res_line,  acc_fake, acc_legitimate
    else:
        acc_val, f1_val, acc_fake, acc_legitimate  = results
        # bug
        # return float(acc_val.item()), float(f1_val.item())
        return float(acc_val), float(f1_val),  acc_fake, acc_legitimate
...
def test(epoch, input_adj_test, input_features_test, idx_out_test, idx_test):
    ...
    if 'multi' in dataset:
        acc_test, res_line,  acc_fake, acc_legitimate  = results
        return float(acc_test.item()), res_line,  acc_fake, acc_legitimate
    else:
        acc_test, f1_test,  acc_fake, acc_legitimate  = results
        # return float(acc_test.item()), float(f1_test.item())
        return float(acc_test), float(f1_test),  acc_fake, acc_legitimate
...
# text,entity,topic之间的连接情况，三种节点的feature（大小为3的list，分别是['text', 'topic', 'entity']的）,被分到训练集的文本索引，被分到验证集的文本索引，被分到测试集的文本索引，idx到index的映射
adj, features, labels, classes_dict, idx_train_ori, idx_val_ori, idx_test_ori, idx_map = load_data(path = path, dataset = dataset)
N = len(adj)
...
# test_dataset = MyDataset(input_adj_test, input_features_test, idx_out_test, idx_test)
# test_dataloader = DataLoader(test_dataset, batch_size=256, shuffle=True, num_workers=8)

FINAL_RESULT = []
for i in range(args.repeat):
# Model and optimizer
    print("\n\nNo. {} test.\n".format(i+1))
    # features是一个大小为3的list，每个list的Size是该类型节点的数量*embedding_size
    ...
    for epoch in range(args.epochs):
        vali_acc, vali_f1, acc_fake, acc_legitimate  = train(epoch,
                         input_adj_train, input_features_train, idx_out_train, idx_train,
                         input_adj_val, input_features_val, idx_out_val, idx_val)
        test_acc, test_f1, acc_fake, acc_legitimate  = test(epoch,
                        input_adj_test, input_features_test, idx_out_test, idx_test)
        if vali_acc > vali_max[0]:
            vali_max = [vali_acc, (test_acc, test_f1),(acc_fake, acc_legitimate), epoch+1]
...
for i in range(len(FINAL_RESULT)):
    if 'multi' in dataset:
        print("{0}:\tvali:  {1:.5f}\ttest:  ACC: {2:.4f},   fake ACC: {4:.4f}, legitimate ACC: {5:.4f}, epoch={6}.\n{3}".format(
            i,
            FINAL_RESULT[i][0],
            FINAL_RESULT[i][1][0],
            FINAL_RESULT[i][1][1],
        ...
        print("{}:\tvali:  {:.5f}\ttest:  ACC: {:.4f} F1: {:.4f}  fake ACC: {:.4f} legitimate ACC: {:.4f}, epoch={}".format(
                        i,
            FINAL_RESULT[i][0],
            FINAL_RESULT[i][1][0],
            FINAL_RESULT[i][1][1],
